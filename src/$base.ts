import { allRows, deleteRow, updateRow } from "./$cache.js";

export type Values<T> = Partial<
  Pick<T, { [K in keyof T]: T[K] extends () => void ? never : K }[keyof T]>
>;
...
export abstract class Base {
  ...
  static async findByIdOrThrow<T extends Base>(
    ...
    if (!val) throw new Error(`No ${this.name} found with id ${id}`);
    return val;
  }
